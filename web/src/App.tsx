        ...
        <div className="position-list">
          {data.positions.map(position => (
            <div className="position-row" key={position.provider}>
              ...
              <div className="position-status"><Pill tone="info">{position.label.includes('demo') ? 'Demo position' : position.status}</Pill><small>Updated {position.reconciledAt}</small></div>
            </div>
          ))}
        </div>
      </section>
...
function System({ data, source, notify, refresh }: { data: DashboardData; source: DataSource; notify: (message: string) => void; refresh: () => void }) {
  ...
  const remaining = data.diagnostics.length - healthy
  const unavailable = data.diagnostics.filter(check => check.status === 'unavailable').length
  const overall = unavailable ? 'Runtime issue' : remaining ? 'Setup incomplete' : 'Runtime ready'
  return (
    <div className="page page-enter">
      ...
      <section className="system-hero"><div className="system-orbit"><span className="orbit orbit-1" /><span className="orbit orbit-2" /><Server size={28} /></div><div><Pill tone={overall === 'Runtime ready' ? 'positive' : unavailable ? 'danger' : 'warning'}><span className="status-dot" />{overall}</Pill><h2>{healthy} operational · {remaining} {remaining === 1 ? 'task' : 'tasks'} remaining.</h2><p>{unavailable ? `${unavailable} runtime checks are unavailable and need attention.` : remaining ? 'The daemon and encrypted ledger are working. Provider supervision, reconciliation, and recovery are tracked setup work—not runtime failures.' : 'All measured local runtime capabilities are operational.'}</p></div><div className="system-facts"><span>Version<strong>{data.version}</strong></span><span>Started<strong>{data.startedAt}</strong></span><span>Account<strong>{data.accountName}</strong></span></div></section>
      <div className="system-grid">
        <section className="panel diagnostics"><SectionHeading eyebrow="Doctor" title="Runtime and release readiness" action={<button className="text-action" onClick={() => { refresh(); notify('Runtime data refreshed') }}><RefreshCw size={13} /> Run again</button>} />{data.diagnostics.map(check => <div className={`diagnostic-row diagnostic-row--${check.status}`} key={check.name}><span>{check.status === 'healthy' ? <Check size={13} /> : <MoreHorizontal size={13} />}</span><div><strong>{check.name}</strong><small>{check.detail}</small></div><Pill tone={check.status === 'healthy' ? 'positive' : check.status === 'attention' ? 'warning' : 'danger'}>{check.label}</Pill></div>)}</section>
